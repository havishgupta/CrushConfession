/* Wordle - fixed listener stacking, dynamic grid */
const WORDLE_ANSWER=()=>CONFIG.yourName.toUpperCase().replace(/\s/g, '');
let wordleState=null;
let _wordleKeyHandler = null;
...
function initWordle(){
    ...
    document.getElementById('wordle-letter-count').textContent = answer.length;
    document.getElementById('wordle-chances').textContent = '5';

    buildWordleGrid();buildWordleKeyboard();
    // Remove previous listener before adding new one (prevents stacking)
    if (_wordleKeyHandler) document.removeEventListener('keydown', _wordleKeyHandler);
    _wordleKeyHandler = handleWordleKey;
    document.addEventListener('keydown', _wordleKeyHandler);
}
...
function handleWordleKey(e){if(!wordleState||wordleState.gameOver)return;const k=e.key.toUpperCase();if(k==='ENTER')submitWordleGuess();else if(k==='BACKSPACE')deleteWordleLetter();else if(/^[A-Z]$/.test(k))addWordleLetter(k);}
...
if(guess===ans){wordleState.gameOver=true;if(_wordleKeyHandler){document.removeEventListener('keydown',_wordleKeyHandler);_wordleKeyHandler=null;}setTimeout(()=>{showScreen('screen-congrats');startCongratsAnimation();},1500);return;}
if(wordleState.currentRow>=5){wordleState.gameOver=true;if(_wordleKeyHandler){document.removeEventListener('keydown',_wordleKeyHandler);_wordleKeyHandler=null;}setTimeout(()=>showGameOverPopup(),1500);}}
