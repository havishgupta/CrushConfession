/* Offer - fixed interval leak, confetti accumulation, red No button */
let _offerInterval = null;
...
function initOffer() {
    // Clear any previous hobby rotation interval
    if (_offerInterval) { clearInterval(_offerInterval); _offerInterval = null; }

    const me = document.getElementById('offer-me');
    if (me && CONFIG.hobbies && CONFIG.hobbies.length > 0) {
        let hi = 0;
        me.textContent = `- ${CONFIG.hobbies[0]}`;
        _offerInterval = setInterval(() => {
    ...
    // Reset button styles on init
    yb.style.transform=''; yb.style.fontSize=''; yb.textContent='Yes!';
    nb.style.transform=''; nb.style.fontSize=''; nb.textContent='No';
    yb.onclick=()=>{if(_offerInterval){clearInterval(_offerInterval);_offerInterval=null;}showScreen('screen-final');initFinalPage();};
    nb.onclick=()=>{nc++;if(nc>=5){if(_offerInterval){clearInterval(_offerInterval);_offerInterval=null;}showGameOverPopup();return;}
